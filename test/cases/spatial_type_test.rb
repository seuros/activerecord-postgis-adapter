...
class SpatialTypeTest < ActiveSupport::TestCase
  def test_point_without_srid
    st_geom = ActiveRecord::PostGIS::OID::STGeometry.new('geometry(Point)')
  ...
  def test_point_with_srid
    st_geom = ActiveRecord::PostGIS::OID::STGeometry.new('geometry(Point,3857)')
  ...
  def test_point_z_without_srid
    st_geom = ActiveRecord::PostGIS::OID::STGeometry.new('geometry(PointZ)')
    assert st_geom.has_z?
    refute st_geom.has_m?
  end

  def test_point_z_m_without_srid
    st_geom = ActiveRecord::PostGIS::OID::STGeometry.new('geometry(PointZM)')
    assert st_geom.has_z?
    assert st_geom.has_m?
  end

  def test_point_z_with_srid
    st_geom = ActiveRecord::PostGIS::OID::STGeometry.new('geometry(PointZM,3857)')
    assert st_geom.has_z?
    assert st_geom.has_m?
    assert_equal 3857, st_geom.srid
  end
  ...
  # Geo
  def test_geo_point
    st_geog = ActiveRecord::PostGIS::OID::STGeography.new('geography(Point)')
  ...
  def test_geo_point_z
    st_geog = ActiveRecord::PostGIS::OID::STGeography.new('geography(PointZ)')
  ...
  def test_geo_point_z_m
    st_geog = ActiveRecord::PostGIS::OID::STGeography.new('geography(PointZM)')
